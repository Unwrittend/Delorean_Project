...
const CHART_WIDTH_FA = 600 - MARGIN.right - MARGIN.left;
const CHART_HEIGHT_FA = 400 - MARGIN.top - MARGIN.bottom;
...
xScale.domain(stuffJson.map((d) => d.time)); // Unique token used to separate data (here, either ID or region can be used)
yScale.domain([ 0, d3.max(stuffJson, d => d.percent)]) // Determines max height of the graph (we add 3 so there's some padding above the tallest bar)
