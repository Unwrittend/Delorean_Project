...
//
// NOTE: THIS FILE IS INCLUDED IN AN HTML FILE SUCH THAT <battery-degredation.js>
//       WILL BE PROCESSED FIRST IN A SHARED GLOBAL SCOPE !!!
//
...
var vehicle_type,
  ...
  flt_profit,
  indiv_profit,
  temperature_in_Kelvin = 293, //TODO mk this variable using Arjun's work
...
function update_CF() {
  ...
  const avgEv_amphrPerYear     = get_ampHr(avgEv_milesDrivenPerYr, avgEv_kWhPerMile);          // from <battery-degredation.js>
  CF = calc_cf_cycle_aging_with_SOC_and_temp(msoc, avgEv_amphrPerYear, temperature_in_Kelvin); // from <battery-degredation.js>
}


//update flt_cap (NOW A JSON VALUE)
...
function calc_flt_cap(years = 5) {
  update_CF(); // required to compute <batt_degrad_factor> below
  const factory_new_flt_cap = veh_pop * (1 - msoc) * battery_capacity; // no battery degredation
  const batt_degrad_factor = Math.min(CF, PF) / 100;
  const batt_degrad_amount = factory_new_flt_cap * batt_degrad_factor;
  flt_cap = [];
  for(let i = 0; i < years; ++i) {
    flt_cap.push({
      'years': i,
      'flt_cap': factory_new_flt_cap - (i * batt_degrad_amount);
    });
  }

  ////////////////////////////////////
  // MOTHBALLING ALL OF CHRIS' CODE //
  ////////////////////////////////////
...
//TODO MK BOTH INSTANCES OF <flt_cap[0]['flt_cap']> RELATIVE TO THE CURRENT YEAR INSTEAD OF ALWAYS THE FIRST (IE BY USING <0>) !!!
/**
...
function calc_revenue_pge_tou(season, division_of_time) {
  ...
  switch (season) {
    case "Winter":
      ...
      var discharged_energy = Math.max(...peak_avail)/100 * flt_cap[0]['flt_cap']; 
