/**
 * NOTE: All percentages are represented as integers, e.g. 4% is represented as 4.0;
 *
 */
function roi( msoc, opt_in, years, battery_capacity, diversity_constant ) {
...
//PGE_TOU_D holds PGE's time of use rates and their on and off peak hours in military time
	const PGE_TOU_D = {
		winter_peak : 0.29231,
...
// CF = Capacity Fade = Battery Degredation
	...
	let CF_per_summer = get_CF_for_season(summerKelvinTemperatures);
	let CF_per_winter = get_CF_for_season(winterKelvinTemperatures);
...
// Return an array of the fleet cpacity for every day of each season with <opt_in> vehicle opt-in values
	function get_season_fltCap_per_day(getFltCapPerSeason, getDailyCapacityDecrements) {
		const fltCap_per_season = getFltCapPerSeason(years+1);
	...
	function get_winter_revenue_pge_tou() {
		const flt_cap_per_day = get_winter_fltCap_per_day(); // array of flt_cap for every day from oct->jan
		let revenue = 0;
		for(let flt_cap of flt_cap_per_day)
	...
	function netSummerRevenueOfOneVehicle() {
		const fleetRevenue = get_summer_revenue_pge_tou();
...
// Note: THIS DOES NOT ACCOUNT FOR REPLACEMENT OF BATTERIES AFTER SOME NUMBER OF SUMMERS
	function netSummerProfitOfOneVehicle() {
		...
		return netSummerRevenueOfOneVehicle() - SUMMER_INDIV_COST_OF_BATTERY_DEGREDATION * years;
	}


	function netWinterRevenueOfOneVehicle() {
		const fleetRevenue = get_winter_revenue_pge_tou();
		const totalCarsInFleet = get_veh_pop();
		return fleetRevenue / totalCarsInFleet;
	}
...
// Note: THIS DOES NOT ACCOUNT FOR REPLACEMENT OF BATTERIES AFTER SOME NUMBER OF WINTERS
	function netWinterProfitOfOneVehicle() {
		...
		return netWinterRevenueOfOneVehicle() - WINTER_INDIV_COST_OF_BATTERY_DEGREDATION * years;
	}
	...
	return({
		fleet: (netSummerProfitOfOneVehicle() + netWinterProfitOfOneVehicle()) * get_veh_pop(),
		cap: getMaxFleetCapacity(),
		data: [
			{season: "Summer", revenue: netSummerProfitOfOneVehicle() * get_veh_pop(), id: 1},
			{season: "Winter", revenue: netWinterProfitOfOneVehicle() * get_veh_pop(), id: 2}
		],
	} );
