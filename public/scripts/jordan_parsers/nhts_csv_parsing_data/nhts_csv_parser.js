...
// Use: designed to parse "NHTS_SPLIT_CSV_FILES/trippub#.csv" files
//   => These files are generated by "nhts_tripub_csv_splitter.cpp"
//   => These files are derived from "trippub.csv" from "https://nhts.ornl.gov/"
...
const CSV_FILEPATH = (n) => `NHTS_SPLIT_CSV_FILES/trippub${n}.csv`;

/******************************************************************************
* READ THE CSV FILE & SPLIT INTO AN ARRAY OF ROWS
******************************************************************************/

function readCsvFile(arr, fileNo) {
  try {
    arr = arr.concat(fs.readFileSync(CSV_FILEPATH(fileNo), 'utf8').split('\n'));
...
function getUseArray() {
  ...
  for(let i = 0; i < TOTAL_MINUTE_SLOTS; ++i) {
    if(!TOTAL_CARS_NOT_CHARGING_THROUGHOUT_DAY[i]) {
      PERCENTAGE_OF_CARS_CHARGING_THROUGHOUT_DAY.push({
        percentage: 0,
        time: i * MINUTES_PER_SLOT,
      });
    } else {
      PERCENTAGE_OF_CARS_CHARGING_THROUGHOUT_DAY.push({
        percentage: 100 * (1 - (TOTAL_CARS_NOT_CHARGING_THROUGHOUT_DAY[i] / TOTAL_CARS)),
        time: i * MINUTES_PER_SLOT,
      });
    }
  }
...
function main() {
  // printObj(getVersion1Data(readCsvRows())); // GENERATES DATA FOR PRESENT
  printObj(getVersion2Data(readCsvRows())); // GENERATES DATA FOR FUTURE
}
